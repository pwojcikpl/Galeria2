class Image < ActiveRecord::Base
  ...
  def validate 
    errors.add_on_empty 'nazwa'
    
    if not (@file.kind_of? StringIO or @file.kind_of? Tempfile)
      errors.add_to_base("Brak pliku z obrazkiem!")
      return
    end
    
    if @file.kind_of? StringIO
      @real_file = Tempfile.new("obr")
      while not @file.eof? 
    ...
    if @jpeg_info == nil or @jpeg_info[0] != 'JPEG' or self.width <= 0 or self.height <= 0
       errors.add_to_base("Obrazek jest uszkodzony, albo w nieprawidłowym formacie (użyj pliku .jpg)")
       return
    end
  end 
